#!/usr/bin/env python3
"""
Low-memory training script to prevent freezing.

This uses quantization and memory optimizations to train on systems with limited RAM.
"""
...
def main():
    """Train the model with low-memory configuration."""
    
    # Default paths
    training_data = Path("data/processed/forum_training_examples.jsonl")
    config_file = Path("training/configs/phi3_mini_low_memory.yaml")
    ...
    # Check if low-memory config exists
    if not config_file.exists():
        print(f"❌ Config file not found: {config_file}")
        print("Creating low-memory config...")
        return 1
    
    print("=" * 60)
    print("LOW-MEMORY TRAINING (Prevents Freezing)")
    print("=" * 60)
    ...
    config['data']['validation_file'] = str(training_data.absolute())
    config['data']['test_file'] = str(training_data.absolute())
    config['training']['output_dir'] = str(output_dir.absolute())
    
    # Save updated config
    temp_config = Path("training/configs/temp_low_memory_training.yaml")
    ...
    print("⚠️  Model loading may take 2-5 minutes. Please be patient!")
    print("=" * 60)
    print()
